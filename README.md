...
## 🌄 9대 서부 테마 배경 시스템 (Dynamic Backdrop Environments)

매 라운드마다 시간대와 장소가 완전히 바뀌는 **9가지 고유 서부 테마 배경 시스템**이 Canvas 렌더러를 통해 실시간으로 그려집니다.

| 라운드 | 테마 명칭 | 시간대 / 천체 | 대표 랜드마크 및 지형 | 분위기 & 색감 |
|:---:|:---|:---|:---|:---|
| **1R** | **더스트 타운** | 작열하는 정오 백색 태양 | 살롱(SALOON), 보안관소(SHERIFF), 풍차, 호텔 | 클래식 서부 메인 스트리트 |
| **2R** | **붉은 협곡** | 오렌지빛 석양 | 거대한 테이블 산(Mesa), 절벽 오두막(SHACK) | 웅장한 사암 메사 밸리 |
| **3R** | **유령 폐광촌** | 먼지에 가려진 희미한 태양 | 갱도 헤드프레임 타워, 광산 제련소(ASSAY) | 황금 붐이 끝난 폐광 유적 |
| **4R** | **철도 교차로** | 청동빛 황혼과 지평선 태양 | 대륙 횡단 2열 철도 레일, 기차역(DEPOT), 전신주 | 광활한 개척 평원 |
| **5R** | **핏빛 사막** | 강렬한 마젠타 핏빛 태양 | 천연 아치 바위(Natural Arch), 모래 언덕 | 피처럼 붉게 타오르는 사막 |
| **6R** | **뇌우의 황무지** | 짙은 뇌운과 내리꽂히는 번개 | 무너진 종탑 교회(CHURCH), 번개 맞은 고목 | 폭풍우 몰아치는 음산한 폐허 |
| **7R** | **황혼의 야영지** | 보랏빛 황혼과 별무리 | 포장마차(Conestoga Wagon), 개척민 텐트 | 낭만적인 밤하늘 초원 |
| **8R** | **부트힐 묘지** | 은빛 보름달(Full Moon) | 묘비 십자가 군락, 부트힐 영묘(Mausoleum) | 차가운 달빛 속 공동묘지 |
| **9R** | **사신의 결전장** | 붉은 코로나 개기일식(Eclipse) | 사신의 교수대(Gallows), 불타버린 폐허 | 아포칼립스 보스 결전장 |

---

## 👑 9라운드 최종 보스 3판 2선승제 (BO3 Raid)

9번째 최종 보스(무법자)와의 마지막 결투는 **3페이즈 3판 2선승제 (Best of 3, First to 2 Wins)** 보스전 모드로 진행됩니다.

* **SET 1 (황야의 조우)**: 기본 보스 사격 스펙 테스트
* **SET 2 (사신의 각성)**: 1세트 패배 시 보스가 분노하여 붉은 오라와 함께 반응속도 +25ms 단축 및 페인트 확률 75%로 증가
* **SET 3 (최후의 일격)**: 1:1 동점 시 펼쳐지는 사투. 반응속도 +45ms 단축 및 페인트 확률 85%
* **실시간 보스 HUD & 전환 연출**: 상단 골드/크림슨 스코어 램프 (`● ○`) 및 세트 간 시네마틱 오버레이 배너
...
5. **보스전 & 시네마틱 컷씬** — 9라운드 3판 2선승제 승리 시 3막 시네마틱 컷씬 및 전설 엔딩 화면 전환. 전적은 `localStorage` (`ai-gunslinger.records.v1`)
